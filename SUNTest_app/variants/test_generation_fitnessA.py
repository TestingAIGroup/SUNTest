...
def get_random_neurons(num_ran_neurons, total_num_neurons):
    s = np.random.choice(range(total_num_neurons), replace=False, size= num_ran_neurons)
    random_neuron_idx = np.sort(s)
    return random_neuron_idx
...
def write2excel(ALL_results, repeat_times, seed_selection_strategy, saved_path):
    ...
    f.save(saved_path + 'MOON_results_FitnessA_200_5_V2.xls')
...
if __name__ == "__main__":
    ...
    trainable_layers = [66]  # cifar10_convnet 和 cfar10_vgg ： target layer都是20
    layer_names = [model.layers[idx].name for idx in trainable_layers]
    model.summary()

    # obtain data
    dataset = 'cifar100_ResNet'
    X_test_corr, Y_test_corr, test_corr_idx = filter_correct_classifications(model, X_test, Y_test)

    # get outputs of EI neurons
    saved_path = '/workspace/EI_Neurons/cifar100_resnet/EI_neurons/'
    EI_neurons_idx = np.load(saved_path + 'cifar100_resnet_tarantula_EI_neurons_idx.npy')
    saved_file_path = '/workspace/EI_Neurons/cifar100_resnet/MOON/MOON-FitnessA/'

    test_ats, test_pred = get_layers_ats(model, X_test, layer_names, dataset)
    total_num_neurons = test_ats.shape[1]

    random_seed = 1
    seeds_num = 200   #种子样本数量
    iteration_max = 5  #迭代次数
    timeout_seconds = 10
    repeat_times = 5 # 重复实验次数
    ...
    seed_root_path = '/workspace/EI_Neurons/cifar100_resnet/seed_index/'

    # 记录所有结果
    ALL_results = {}

    Random_seed_index = {}
    Random_seed_index[0] = np.load(seed_root_path + 'Random_seed_index_V2_0.npy')
    Random_seed_index[1] = np.load(seed_root_path + 'Random_seed_index_V2_1.npy')
    Random_seed_index[2] = np.load(seed_root_path + 'Random_seed_index_V2_2.npy')
    Random_seed_index[3] = np.load(seed_root_path + 'Random_seed_index_V2_3.npy')
    Random_seed_index[4] = np.load(seed_root_path + 'Random_seed_index_V2_4.npy')
    ...
    Deepgini_seed_index = {}
    Deepgini_seed_index[0] = np.load(seed_root_path + 'Deepgini_seed_index_V2_0.npy')
    Deepgini_seed_index[1] = np.load(seed_root_path + 'Deepgini_seed_index_V2_1.npy')
    Deepgini_seed_index[2] = np.load(seed_root_path + 'Deepgini_seed_index_V2_2.npy')
    Deepgini_seed_index[3] = np.load(seed_root_path + 'Deepgini_seed_index_V2_3.npy')
    Deepgini_seed_index[4] = np.load(seed_root_path + 'Deepgini_seed_index_V2_4.npy')

    MCP_seed_index = {}
    MCP_seed_index[0] = np.load(seed_root_path + 'mcp_seed_index_V2_0.npy')
    MCP_seed_index[1] = np.load(seed_root_path + 'mcp_seed_index_V2_1.npy')
    MCP_seed_index[2] = np.load(seed_root_path + 'mcp_seed_index_V2_2.npy')
    MCP_seed_index[3] = np.load(seed_root_path + 'mcp_seed_index_V2_3.npy')
    MCP_seed_index[4] = np.load(seed_root_path + 'mcp_seed_index_V2_4.npy')
    ...
    for repeat in range(0, repeat_times):
        ...
        for seed_strategy in seed_selection_strategy:
            ...
            if seed_strategy == 'Random':
                ...
                print('Random strategy selected seeds, top_indices:', len(top_indices))

            elif seed_strategy == 'Deepgini':

                top_indices = Deepgini_seed_index[repeat]
                print('Deepgini strategy selected seeds, top_indices:', len(top_indices))
            ...
            elif seed_strategy == 'RobotBe':

                top_indices = BE_seed_index[repeat]
                print('Robot-Best strategy selected seeds, top_indices:', len(top_indices))

            elif seed_strategy == 'RobotKM':
                top_indices = KM_seed_index[repeat]
                print('Robot-KM strategy selected seeds, top_indices:', len(top_indices))
            ...
            for idx in top_indices:
                ...
                for iteration in range(1, iteration_max + 1):
                    # if time.time() - seed_start_time > timeout_seconds:
                    #     print('Time Out--------------------------------------')
                    #     break
            ...
            np.savez(saved_file_path + 'MOON_all_gen_inps_FitnessA_{}_strategy_{}_repeat_V2.npz'.format(seed_strategy, str(repeat)),
