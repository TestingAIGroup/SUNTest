...
def write2excel(ALL_results, repeat_times, seed_selection_strategy, saved_path):
    ...
    for repeat in range(0, repeat_times):
        ...
        all_seed_strategy_summary_list = ALL_results[repeat]
    ...
    f.save(saved_path + 'MOON_fitC_results_RandomNeuron_{}Seed_NewMutationRankings_200_5_V1.xls'.format(seed_selection_strategy))
...
if __name__ == "__main__":
    ...
    trainable_layers = [66]  # cifar100_vgg target_layer 是41
    print('model.target_leyers: ', model.layers[66].name)
    ...
    saved_file_path = '/workspace/EI_Neurons/cifar100_resnet/MOON/MOON-Random-Neuron/'
    ...
    BE_seed_index = {}
    BE_seed_index[0] = np.load(seed_root_path + 'Be_seed_index_V1_0.npy')
    BE_seed_index[1] = np.load(seed_root_path + 'Be_seed_index_V1_1.npy')
    BE_seed_index[2] = np.load(seed_root_path + 'Be_seed_index_V1_2.npy')
    BE_seed_index[3] = np.load(seed_root_path + 'Be_seed_index_V1_3.npy')
    BE_seed_index[4] = np.load(seed_root_path + 'Be_seed_index_V1_4.npy')

    KM_seed_index = {}
    KM_seed_index[0] = np.load(seed_root_path + 'KM_seed_index_V1_0.npy')
    KM_seed_index[1] = np.load(seed_root_path + 'KM_seed_index_V1_1.npy')
    KM_seed_index[2] = np.load(seed_root_path + 'KM_seed_index_V1_2.npy')
    KM_seed_index[3] = np.load(seed_root_path + 'KM_seed_index_V1_3.npy')
    KM_seed_index[4] = np.load(seed_root_path + 'KM_seed_index_V1_4.npy')
    ...
    for repeat in range(0, repeat_times):
        ...
        for seed_strategy in seed_selection_strategy:
            ...
            for idx in top_indices:
                ...
                for iteration in range(1, iteration_max + 1):
                    ...
                    if mutated_seed_label != raw_seed_label:
                        ...
                        ratio = 0.80
                        num_ran_neurons = round(len(EI_neurons_idx) * ratio)
                        final_neuron_idx = get_random_neurons(num_ran_neurons, total_num_neurons)

                        new_test_ats, new_test_pred = get_layers_new_img_ats(model, new_img, layer_names, dataset)
                        img_EI_neurons_ats = new_test_ats[:, final_neuron_idx]
                        img_EI_neurons_ats_value = np.sum(new_test_ats[:, final_neuron_idx], axis=1)  # [11.986867]
            ...
            ALL_results[repeat].append(summary_str)

            # 生成的inputs存储在文件中
            trigger_obj.all_gen_inps  = {str(key): value for key, value in trigger_obj.all_gen_inps.items()}

            np.savez(saved_file_path + 'MOON_all_gen_inps_RandomNeuron_{}_strategy_{}_repeat_V1.npz'.format(seed_strategy, str(repeat)),
